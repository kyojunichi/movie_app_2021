...
## 조건부 렌더링
- React에서는 원하는 동작(動作)를 캡슐화(化)하는 컴포넌트를 만들 수 있다。
- 애플리케이션의 상태(狀態)에 따라서 컴포넌트 中 몇 개(個)만을 렌더링할 수 있다。
- React에서 조건부(條件部)렌더링은 JavaScript에서의 조건처리(條件處理)와 동일(同一)하게 동작(動作)한다。
